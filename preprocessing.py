...
if __name__ == "__main__":
  ...
  parser.add_argument('--data_path', type=str,
                      help='Path to videos dataset')

  parser.add_argument('--img_out_path', type=str,
                      help='Path to dir contains output all images frame')
  args = parser.parse_args()



  filenames = next(os.walk(os.path.join(ROOT_DIR,args.data_path)), (None, None, []))[2]
  print(os.path.join(ROOT_DIR,'label.csv'))
  rgb_out_dir = os.path.join(ROOT_DIR,args.img_out_path)
  print(rgb_out_dir)
  if not os.path.exists(rgb_out_dir):
      os.makedirs(rgb_out_dir)
      
  d_out_dir = ''
  label_csv = read_csv(os.path.join(ROOT_DIR,args.data_path,'label.csv'))
  vids_path = os.path.join(ROOT_DIR,args.data_path,'videos/')
  ...
  out_zip = os.path.join(ROOT_DIR, rgb_out_dir)
  print('Zipping image dataset...')
  shutil.make_archive(out_zip,'zip',rgb_out_dir)
  print('Done!')

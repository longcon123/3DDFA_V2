...
def ser_to_ply_single(ver_lst, tri, height, wfp, reverse=True):
    ...
    for i, ver in enumerate(ver_lst):
        wfp_new = wfp.replace(suffix, '_{}{}'.format(i + 1,suffix))
        ...
        with open(wfp_new, 'w') as f:
            ...
            for i in range(n_face):
                ...
                if reverse:
                    f.write('3 {idx3} {idx2} {idx1}\n')
                else:
                    f.write('3 {idx1} {idx2} {idx3}\n')

        print('Dump tp {wfp_new}')
    ...
    with open(wfp, 'w') as f:
        ...
        for i in range(n_ply):
            ...
            for j in range(n_vertex):
                x, y, z = ver[:, j]
                if reverse:
                    f.write('{x:.2f} {height - y:.2f} {z:.2f}\n')
                else:
                    f.write('{x:.2f} {y:.2f} {z:.2f}\n')
        ...
        for i in range(n_ply):
            ...
            for j in range(n_face):
                ...
                if reverse:
                    f.write('3 {idx3 + offset} {idx2 + offset} {idx1 + offset}\n')
                else:
                    f.write('3 {idx1 + offset} {idx2 + offset} {idx3 + offset}\n')
...
def ser_to_obj_single(img, ver_lst, tri, height, wfp):
    ...
    for i, ver in enumerate(ver_lst):
        ...
        wfp_new = wfp.replace(suffix, '_{i + 1}{suffix}')

        with open(wfp_new, 'w') as f:
            for i in range(n_vertex):
                x, y, z = ver[:, i]
                f.write(
                    'v {x:.2f} {height - y:.2f} {z:.2f} {colors[i, 2]:.2f} {colors[i, 1]:.2f} {colors[i, 0]:.2f}\n')
            for i in range(n_face):
                idx1, idx2, idx3 = tri[i]  # m x 3
                f.write('f {idx3 + 1} {idx2 + 1} {idx1 + 1}\n')
    ...
    with open(wfp, 'w') as f:
        for i in range(n_obj):
            ...
            for j in range(n_vertex):
                x, y, z = ver[:, j]
                f.write(
                    'v {x:.2f} {height - y:.2f} {z:.2f} {colors[j, 2]:.2f} {colors[j, 1]:.2f} {colors[j, 0]:.2f}\n')

        for i in range(n_obj):
            offset = i * n_vertex
            for j in range(n_face):
                idx1, idx2, idx3 = tri[j]  # m x 3
                f.write('f {idx3 + 1 + offset} {idx2 + 1 + offset} {idx1 + 1 + offset}\n')
